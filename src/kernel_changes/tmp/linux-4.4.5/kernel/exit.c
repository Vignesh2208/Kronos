...
static void __exit_signal(struct task_struct *tsk) {
  ...
  sighand =
      rcu_dereference_check(tsk->sighand, lockdep_tasklist_lock_is_held());
  ...
  if (group_dead) {
     ...
     */
    if (unlikely(has_group_leader_pid(tsk))) posix_cpu_timers_exit_group(tsk);
...
static int will_become_orphaned_pgrp(struct pid *pgrp,
  ...
  do_each_pid_task(pgrp, PIDTYPE_PGID, p) {
    if ((p == ignored_task) || (p->exit_state && thread_group_empty(p)) ||
...
static struct task_struct *find_alive_thread(struct task_struct *p) {
  ...
  for_each_thread(p, t) {
    if (!(t->flags & PF_EXITING)) return t;
  }
  return NULL;
}
...
static struct task_struct *find_new_reaper(struct task_struct *father,
  ...
  if (father->signal->has_child_subreaper) {
     ...
     */
    for (reaper = father; !same_thread_group(reaper, child_reaper);
...
static void forget_original_parent(struct task_struct *father,
  ...
  if (unlikely(!list_empty(&father->ptraced))) exit_ptrace(father, dead);

  /* Can drop and reacquire tasklist_lock */
  reaper = find_child_reaper(father);
  if (list_empty(&father->children)) return;
  ...
  list_for_each_entry(p, &father->children, sibling) {
    for_each_thread(p, t) {
      ...
      if (likely(!t->ptrace)) t->parent = t->real_parent;
      if (t->pdeath_signal)
...
void do_exit(long code) {
  ...
  if (unlikely(in_atomic())) {
    pr_info("note: %s[%d] exited with preempt_count %d\n", current->comm,
            task_pid_nr(current), preempt_count());
...
static inline struct pid *task_pid_type(struct task_struct *task,
                                        enum pid_type type) {
...
static int wait_noreap_copyout(struct wait_opts *wo, struct task_struct *p,
  ...
  int retval = wo->wo_rusage ? getrusage(p, RUSAGE_BOTH, wo->wo_rusage) : 0;
  ...
  if (infop) {
    ...
    if (!retval) retval = put_user(status, &infop->si_status);
  }
...
static int wait_task_zombie(struct wait_opts *wo, struct task_struct *p) {
   ...
   */
  state =
      (ptrace_reparented(p) && thread_group_leader(p)) ? EXIT_TRACE : EXIT_DEAD;
  if (cmpxchg(&p->exit_state, EXIT_ZOMBIE, state) != EXIT_ZOMBIE) return 0;
  /*
  ...
  if (state == EXIT_DEAD && thread_group_leader(p)) {
    ...
    if (psig->cmaxrss < maxrss) psig->cmaxrss = maxrss;
  ...
  status = (p->signal->flags & SIGNAL_GROUP_EXIT) ? p->signal->group_exit_code
                                                  : p->exit_code;
  if (!retval && wo->wo_stat) retval = put_user(status, wo->wo_stat);

  infop = wo->wo_info;
  if (!retval && infop) retval = put_user(SIGCHLD, &infop->si_signo);
  if (!retval && infop) retval = put_user(0, &infop->si_errno);
  if (!retval && infop) {
    int why;
  ...
  if (!retval && infop) retval = put_user(pid, &infop->si_pid);
  if (!retval && infop) retval = put_user(uid, &infop->si_uid);
  if (!retval) retval = pid;
  ...
  if (state == EXIT_TRACE) {
    ...
    state = EXIT_ZOMBIE;
    if (do_notify_parent(p, p->exit_signal)) state = EXIT_DEAD;
    p->exit_state = state;
    write_unlock_irq(&tasklist_lock);
  }
...
static int wait_task_stopped(struct wait_opts *wo, int ptrace,
  ...
  if (unlikely(!p_code)) goto unlock_sig;

  exit_code = *p_code;
  if (!exit_code) goto unlock_sig;

  if (!unlikely(wo->wo_flags & WNOWAIT)) *p_code = 0;
...
unlock_sig:
  ...
  if (!retval && infop) retval = put_user(SIGCHLD, &infop->si_signo);
  if (!retval && infop) retval = put_user(0, &infop->si_errno);
  if (!retval && infop) retval = put_user((short)why, &infop->si_code);
  if (!retval && infop) retval = put_user(exit_code, &infop->si_status);
  if (!retval && infop) retval = put_user(pid, &infop->si_pid);
  if (!retval && infop) retval = put_user(uid, &infop->si_uid);
  if (!retval) retval = pid;
  put_task_struct(p);
...
// if ptrace = 1, then p is traced by current
static int wait_consider_task(struct wait_opts *wo, int ptrace,
  ...
  if (unlikely(exit_state == EXIT_TRACE)) {
     ...
     */
    if (likely(!ptrace)) wo->notask_error = 0;
    return 0;
  }
  ...
  if (likely(!ptrace) && unlikely(p->ptrace)) {
     ...
     */
    if (!ptrace_reparented(p)) ptrace = 1;
  }
...
static int child_wait_callback(wait_queue_t *wait, unsigned mode, int sync,
                               void *key) {
...
repeat:
  ...
  if (child != NULL && wo->wo_flags & WTRACE_DESCENDENTS) {
    if (current->pid != init_task.pid && current->virt_start_time == 0 &&
        child != NULL &&
...
SYSCALL_DEFINE5(waitid, int, which, pid_t, upid, struct siginfo __user *, infop,
  ...
  if (ret > 0) {
     ...
     * difference.
     */
    if (!ret) ret = put_user(0, &infop->si_signo);
    if (!ret) ret = put_user(0, &infop->si_errno);
    if (!ret) ret = put_user(0, &infop->si_code);
    if (!ret) ret = put_user(0, &infop->si_pid);
    if (!ret) ret = put_user(0, &infop->si_uid);
    if (!ret) ret = put_user(0, &infop->si_status);
  }
...
SYSCALL_DEFINE4(wait4, pid_t, upid, int __user *, stat_addr, int, options,
  ...
  if (options & ~(WNOHANG | WUNTRACED | WCONTINUED | __WNOTHREAD | __WCLONE |
                  __WALL | WTRACE_DESCENDENTS))
    return -EINVAL;
