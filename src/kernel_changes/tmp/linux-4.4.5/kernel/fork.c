...
int nr_processes(void) {
  ...
  for_each_possible_cpu(cpu) total += per_cpu(process_counts, cpu);
...
void thread_info_cache_init(void) {
  thread_info_cache =
      kmem_cache_create("thread_info", THREAD_SIZE, THREAD_SIZE, 0, NULL);
...
static inline void put_signal_struct(struct signal_struct *sig) {
  if (atomic_dec_and_test(&sig->sigcnt)) free_signal_struct(sig);
}
...
static void set_max_threads(unsigned int max_threads_suggested) {
  ...
  if (fls64(totalram_pages) + fls64(PAGE_SIZE) > 64)
    ...
    threads =
        div64_u64((u64)totalram_pages * (u64)PAGE_SIZE, (u64)THREAD_SIZE * 8UL);
...
static struct task_struct *dup_task_struct(struct task_struct *orig) {
  ...
  tsk->burst_target = orig->burst_target;
  tsk->overshoot_error = orig->overshoot_error;
...
static int dup_mmap(struct mm_struct *mm, struct mm_struct *oldmm) {
  ...
  for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
    ...
    *tmp = *mpnt;
    ...
    if (retval) goto fail_nomem_policy;
    tmp->vm_mm = mm;
    ...
    if (file) {
      ...
      get_file(file);
      if (tmp->vm_flags & VM_DENYWRITE) atomic_dec(&inode->i_writecount);
      i_mmap_lock_write(mapping);
      if (tmp->vm_flags & VM_SHARED) atomic_inc(&mapping->i_mmap_writable);
     ...
     */
    if (is_vm_hugetlb_page(tmp)) reset_vma_resv_huge_pages(tmp);
...
static int __init coredump_filter_setup(char *s) {
  default_dump_filter = (simple_strtoul(s, NULL, 0) << MMF_DUMP_FILTER_SHIFT) &
...
static struct mm_struct *mm_init(struct mm_struct *mm, struct task_struct *p) {
  ...
  if (mm_alloc_pgd(mm)) goto fail_nopgd;

  if (init_new_context(p, mm)) goto fail_nocontext;
...
void set_mm_exe_file(struct mm_struct *mm, struct file *new_exe_file) {
  ...
  if (old_exe_file) fput(old_exe_file);
}
...
struct file *get_mm_exe_file(struct mm_struct *mm) {
  ...
  if (exe_file && !get_file_rcu(exe_file)) exe_file = NULL;
  rcu_read_unlock();
  return exe_file;
}
...
/* Get rid of any futexes when releasing the mm */
   ...
   */
  if (tsk->vfork_done) complete_vfork_done(tsk);
}
...
static int copy_io(unsigned long clone_flags, struct task_struct *tsk) {
  ...
  if (clone_flags & CLONE_IO) {
    ...
    if (unlikely(!new_ioc)) return -ENOMEM;
...
static void copy_seccomp(struct task_struct *p) {
   ...
   */
  if (task_no_new_privs(current)) task_set_no_new_privs(p);
...
static struct task_struct *copy_process(unsigned long clone_flags,
   ...
   */
  p->clear_child_tid =
      (clone_flags & CLONE_CHILD_CLEARTID) ? child_tidptr : NULL;
#ifdef CONFIG_BLOCK
  p->plug = NULL;
#endif
#ifdef CONFIG_FUTEX
...
bad_fork_cleanup_signal:
  if (!(clone_flags & CLONE_THREAD)) free_signal_struct(p->signal);
...
long _do_fork(unsigned long clone_flags, unsigned long stack_start,
  ...
  if (!IS_ERR(p)) {
    ...
    if (unlikely(trace)) {
      trace_printk(
          "Sending NEW CLONE/FORK event to tracer. Current task : %d, "
          "Event = %d\n",
          current->pid, trace);
      ptrace_event_pid(trace, pid);
      trace_printk(
          "Resuming after CLONE/FORK event send. Current task : %d, "
          "Event content = %lu\n",
          current->pid, current->ptrace_message);
    }
...
long do_fork(unsigned long clone_flags, unsigned long stack_start,
             unsigned long stack_size, int __user *parent_tidptr,
             int __user *child_tidptr) {
  return _do_fork(clone_flags, stack_start, stack_size, parent_tidptr,
                  child_tidptr, 0);
}
#endif
...
void __init proc_caches_init(void) {
  sighand_cachep = kmem_cache_create(
      ...
      SLAB_HWCACHE_ALIGN | SLAB_PANIC | SLAB_DESTROY_BY_RCU | SLAB_NOTRACK,
      sighand_ctor);
  signal_cachep =
      kmem_cache_create("signal_cache", sizeof(struct signal_struct), 0,
                        SLAB_HWCACHE_ALIGN | SLAB_PANIC | SLAB_NOTRACK, NULL);
  files_cachep =
      kmem_cache_create("files_cache", sizeof(struct files_struct), 0,
                        SLAB_HWCACHE_ALIGN | SLAB_PANIC | SLAB_NOTRACK, NULL);
  fs_cachep =
...
static int check_unshare_flags(unsigned long unshare_flags) {
  ...
  if (unshare_flags & (CLONE_SIGHAND | CLONE_VM)) {
    if (atomic_read(&current->sighand->count) > 1) return -EINVAL;
  }
...
static int unshare_fs(unsigned long unshare_flags, struct fs_struct **new_fsp) {
  ...
  if (!(unshare_flags & CLONE_FS) || !fs) return 0;

  /* don't need lock here; in the worst case we'll do useless copy */
  if (fs->users == 1) return 0;

  *new_fsp = copy_fs_struct(fs);
  if (!*new_fsp) return -ENOMEM;
...
SYSCALL_DEFINE1(unshare, unsigned long, unshare_flags) {
   ...
   */
  if (unshare_flags & (CLONE_NEWIPC | CLONE_SYSVSEM)) do_sysvsem = 1;
  err = unshare_fs(unshare_flags, &new_fs);
  if (err) goto bad_unshare_out;
  err = unshare_fd(unshare_flags, &new_fd);
  if (err) goto bad_unshare_cleanup_fs;
  err = unshare_userns(unshare_flags, &new_cred);
  if (err) goto bad_unshare_cleanup_fd;
  err =
  ...
  if (new_fs || new_fd || do_sysvsem || new_cred || new_nsproxy) {
    ...
    if (new_nsproxy) switch_task_namespaces(current, new_nsproxy);
...
bad_unshare_cleanup_fd:
  if (new_fd) put_files_struct(new_fd);
...
int sysctl_max_threads(struct ctl_table *table, int write, void __user *buffer,
