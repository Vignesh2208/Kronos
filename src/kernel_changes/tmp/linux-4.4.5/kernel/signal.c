...
static int recalc_sigpending_tsk(struct task_struct *t) {
  if ((t->jobctl & JOBCTL_PENDING_MASK) || PENDING(&t->pending, &t->blocked) ||
...
void recalc_sigpending_and_wake(struct task_struct *t) {
  if (recalc_sigpending_tsk(t)) signal_wake_up(t, 0);
}
...
int next_signal(struct sigpending *pending, sigset_t *mask) {
  ...
  x = *s & ~*m;
  if (x) {
    if (x & SYNCHRONOUS_MASK) x &= SYNCHRONOUS_MASK;
    sig = ffz(~x) + 1;
    return sig;
  }
...
static inline void print_dropped_signal(int sig) {
  ...
  if (!print_fatal_signals) return;

  if (!__ratelimit(&ratelimit_state)) return;
...
bool task_set_jobctl_pending(struct task_struct *task, unsigned long mask) {
  ...
  if (mask & JOBCTL_STOP_SIGMASK) task->jobctl &= ~JOBCTL_STOP_SIGMASK;
...
static bool task_participate_group_stop(struct task_struct *task) {
  ...
  if (!WARN_ON_ONCE(sig->group_stop_count == 0)) sig->group_stop_count--;
...
void ignore_signals(struct task_struct *t) {
  ...
  for (i = 0; i < _NSIG; ++i) t->sighand->action[i].sa.sa_handler = SIG_IGN;
...
int unhandled_signal(struct task_struct *tsk, int sig) {
  ...
  if (is_global_init(tsk)) return 1;
...
static inline bool si_fromuser(const struct siginfo *info) {
  return info == SEND_SIG_NOINFO || (!is_si_special(info) && SI_FROMUSER(info));
}
...
static int kill_ok_by_cred(struct task_struct *t) {
  ...
  if (ns_capable(tcred->user_ns, CAP_KILL)) return 1;
...
static void complete_signal(int sig, struct task_struct *p, int group) {
  ...
  if (sig_fatal(p, sig) &&
      ...
      !sigismember(&t->real_blocked, sig) && (sig == SIGKILL || !t->ptrace)) {
    /*
...
static inline void userns_fixup_signal_uid(struct siginfo *info,
                                           struct task_struct *t) {
  if (current_user_ns() == task_cred_xxx(t, user_ns)) return;

  if (SI_FROMKERNEL(info)) return;
...
static int __send_signal(int sig, struct siginfo *info, struct task_struct *t,
  ...
  if (legacy_queue(pending, sig)) goto ret;
...
int __group_send_sig_info(int sig, struct siginfo *info,
...
int kill_pid_info(int sig, struct siginfo *info, struct pid *pid) {
  ...
  for (;;) {
    ...
    if (p) error = group_send_sig_info(sig, info, p);
    rcu_read_unlock();
    if (likely(!p || error != -ESRCH)) return error;
...
int send_sigqueue(struct sigqueue *q, struct task_struct *t, int group) {
  ...
  ret = -1;
  if (!likely(lock_task_sighand(t, &flags))) goto ret;

  ret = 1; /* the signal is ignored */
  result = TRACE_SIGNAL_IGNORED;
  if (!prepare_signal(sig, t, false)) goto out;
...
bool do_notify_parent(struct task_struct *tsk, int sig) {
  ...
  info.si_uid =
      from_kuid_munged(task_cred_xxx(tsk->parent, user_ns), task_uid(tsk));
  rcu_read_unlock();
...
static inline int may_ptrace_stop(void) {
  if (!likely(current->ptrace)) return 0;
  /*
...
static bool do_signal_stop(int signr) __releases(&current->sighand->siglock) {
  ...
  if (likely(!current->ptrace)) {
     ...
     */
    if (task_participate_group_stop(current)) notify = CLD_STOPPED;
...
static void do_jobctl_trap(void) {
  ...
  if (current->ptrace & PT_SEIZED) {
    if (!signal->group_stop_count && !(signal->flags & SIGNAL_STOP_STOPPED))
      signr = SIGTRAP;
    WARN_ON_ONCE(!signr);
    ptrace_do_notify(signr, signr | (PTRACE_EVENT_STOP << 8), CLD_STOPPED);
  } else {
    WARN_ON_ONCE(!signr);
...
static int ptrace_signal(int signr, siginfo_t *info) {
  ...
  if (signr != info->si_signo) {
    ...
    info->si_uid =
        from_kuid_munged(current_user_ns(), task_uid(current->parent));
    rcu_read_unlock();
  }
...
relock:
  ...
  for (;;) {
    ...
    if (sig_kernel_coredump(signr)) {
      if (print_fatal_signals) print_fatal_signal(ksig->info.si_signo);
...
simply clear the restore sigmask flag.  */
  ...
  if (!(ksig->ka.sa.sa_flags & SA_NODEFER)) sigaddset(&blocked, ksig->sig);
...
static void retarget_shared_pending(struct task_struct *tsk, sigset_t *which) {
  ...
  if (sigisemptyset(&retarget)) return;

  t = tsk;
  while_each_thread(tsk, t) {
    if (t->flags & PF_EXITING) continue;

    if (!has_pending_signals(&retarget, &t->blocked)) continue;
    /* Remove the signals this thread can handle. */
    sigandsets(&retarget, &retarget, &t->blocked);

    if (!signal_pending(t)) signal_wake_up(t, 0);

    if (sigisemptyset(&retarget)) break;
  }
}
...
static void __set_task_blocked(struct task_struct *tsk,
...
SYSCALL_DEFINE4(rt_sigprocmask, int, how, sigset_t __user *, nset,
  ...
  if (nset) {
    if (copy_from_user(&new_set, nset, sizeof(sigset_t))) return -EFAULT;
  ...
  if (oset) {
    if (copy_to_user(oset, &old_set, sizeof(sigset_t))) return -EFAULT;
  }
...
COMPAT_SYSCALL_DEFINE4(rt_sigprocmask, int, how, compat_sigset_t __user *, nset,
  ...
  if (nset) {
    ...
    sigset_t new_set;
    int error;
    if (copy_from_user(&new32, nset, sizeof(compat_sigset_t))) return -EFAULT;
  ...
  if (oset) {
    ...
    if (copy_to_user(oset, &old32, sizeof(compat_sigset_t))) return -EFAULT;
  }
  return 0;
#else
...
SYSCALL_DEFINE2(rt_sigpending, sigset_t __user *, uset, size_t, sigsetsize) {
  ...
  if (!err && copy_to_user(uset, &set, sigsetsize)) err = -EFAULT;
  return err;
}
...
COMPAT_SYSCALL_DEFINE2(rt_sigpending, compat_sigset_t __user *, uset,
  ...
  if (!err) {
    ...
    if (copy_to_user(uset, &set32, sigsetsize)) err = -EFAULT;
  }
  return err;
#else
...
int copy_siginfo_to_user(siginfo_t __user *to, const siginfo_t *from) {
  ...
  if (!access_ok(VERIFY_WRITE, to, sizeof(siginfo_t))) return -EFAULT;
  if (from->si_code < 0)
    return __copy_to_user(to, from, sizeof(siginfo_t)) ? -EFAULT : 0;
  /*
...
int do_sigtimedwait(const sigset_t *which, siginfo_t *info,
  ...
  if (ts) {
     ...
     */
    if (ts->tv_sec || ts->tv_nsec) timeout++;
  }
...
static int do_send_specific(pid_t tgid, pid_t pid, int sig,
  ...
  if (p && (tgid <= 0 || task_tgid_vnr(p) == tgid)) {
    ...
    if (!error && sig) {
       ...
       */
      if (unlikely(error == -ESRCH)) error = 0;
    }
  }
  rcu_read_unlock();
...
#ifdef CONFIG_COMPAT
COMPAT_SYSCALL_DEFINE3(rt_sigqueueinfo, compat_pid_t, pid, int, sig,
...
static int do_rt_tgsigqueueinfo(pid_t tgid, pid_t pid, int sig,
                                siginfo_t *info) {
  /* This is only valid for single tasks */
  if (pid <= 0 || tgid <= 0) return -EINVAL;
...
#ifdef CONFIG_COMPAT
COMPAT_SYSCALL_DEFINE4(rt_tgsigqueueinfo, compat_pid_t, tgid, compat_pid_t, pid,
                       int, sig, struct compat_siginfo __user *, uinfo) {
  siginfo_t info = {};

  if (copy_siginfo_from_user32(&info, uinfo)) return -EFAULT;
  return do_rt_tgsigqueueinfo(tgid, pid, sig, &info);
}
#endif
  ...
  if (act) {
    ...
    *k = *act;
    ...
    if (sig_handler_ignored(sig_handler(p, sig), sig)) {
      ...
      for_each_thread(p, t) flush_sigqueue_mask(&mask, &t->pending);
    }
  }
...
static int do_sigaltstack(const stack_t __user *uss, stack_t __user *uoss,
  ...
  if (uss) {
    ...
    error = -EFAULT;
    if (!access_ok(VERIFY_READ, uss, sizeof(*uss))) goto out;
    ...
    if (ss_flags == SS_DISABLE) {
      ...
      error = -ENOMEM;
      if (ss_size < MINSIGSTKSZ) goto out;
    }
  ...
  if (uoss) {
    error = -EFAULT;
    if (!access_ok(VERIFY_WRITE, uoss, sizeof(*uoss))) goto out;
...
#ifdef CONFIG_COMPAT
COMPAT_SYSCALL_DEFINE2(sigaltstack, const compat_stack_t __user *, uss_ptr,
  ...
  if (uss_ptr) {
    ...
    if (copy_from_user(&uss32, uss_ptr, sizeof(compat_stack_t))) return -EFAULT;
...
SYSCALL_DEFINE3(sigprocmask, int, how, old_sigset_t __user *, nset,
  ...
  if (nset) {
    if (copy_from_user(&new_set, nset, sizeof(*nset))) return -EFAULT;
...
SYSCALL_DEFINE4(rt_sigaction, int, sig, const struct sigaction __user *, act,
  ...
  if (act) {
    if (copy_from_user(&new_sa.sa, act, sizeof(new_sa.sa))) return -EFAULT;
  }

  ret = do_sigaction(sig, act ? &new_sa : NULL, oact ? &old_sa : NULL);

  if (!ret && oact) {
    if (copy_to_user(oact, &old_sa.sa, sizeof(old_sa.sa))) return -EFAULT;
  }
out:
  return ret;
}
...
COMPAT_SYSCALL_DEFINE4(rt_sigaction, int, sig,
                       ...
                       struct compat_sigaction __user *, oact, compat_size_t,
                       sigsetsize) {
...
__weak const char *arch_vma_name(struct vm_area_struct *vma) { return NULL; }
