...
static bool loss_4state(struct netem_sched_data *q) {
   ...
   *   TX_IN_GAP_PERIOD => successfully transmitted packets within a burst
   * period
   */
  switch (clg->state) {
...
static bool loss_gilb_ell(struct netem_sched_data *q) {
  ...
  switch (clg->state) {
    case GOOD_STATE:
      if (prandom_u32() < clg->a1) clg->state = BAD_STATE;
      if (prandom_u32() < clg->a4) return true;
      break;
    case BAD_STATE:
      if (prandom_u32() < clg->a2) clg->state = GOOD_STATE;
      if (prandom_u32() > clg->a3) return true;
  }
...
static psched_tdiff_t tabledist(psched_tdiff_t mu, psched_tdiff_t sigma,
  ...
  if (dist == NULL) return (rnd % (2 * sigma)) - sigma + mu;
...
static int netem_enqueue(struct sk_buff *skb, struct Qdisc *sch) {
   ...
   */
  if (q->latency || q->jitter) skb_orphan_partial(skb);
  ...
  if (q->gap == 0 ||             /* not doing reordering */
    ...
    if (q->rate &&
        ts == NULL) {  //(TODO) rate limiting is not supported under TimeKeeper
...
static struct sk_buff *netem_dequeue(struct Qdisc *sch) {
  ...
  if (qdisc_is_throttled(sch)) return NULL;
...
tfifo_dequeue:
  ...
  if (p) {
    ...
    if (ts == NULL) {
      ...
      if (q->jitter == 0 && netem_skb_cb(skb)->tstamp_save.tv64 != 0) {
        time_to_send = PSCHED_NS2TICKS(netem_skb_cb(skb)->tstamp_save.tv64);
    ...
    if (time_to_send <= curr_time + 1) {
      ...
      if (netem_skb_cb(skb)->tstamp_save.tv64 == 0 && q->jitter == 0 &&
          ts != NULL) {
        netem_skb_cb(skb)->tstamp_save.tv64 =
            PSCHED_TICKS2NS(time_to_send - q->latency);
      }
...
static int get_dist_table(struct Qdisc *sch, const struct nlattr *attr) {
  ...
  if (n > NETEM_DIST_MAX) return -EINVAL;
  ...
  d->size = n;
  for (i = 0; i < n; i++) d->table[i] = data[i];
...
static void get_correlation(struct netem_sched_data *q,
...
static int parse_attr(struct nlattr *tb[], int maxtype, struct nlattr *nla,
  ...
  if (nested_len >= nla_attr_size(0))
    return nla_parse(tb, maxtype, nla_data(nla) + NLA_ALIGN(len), nested_len,
                     policy);
...
static int netem_change(struct Qdisc *sch, struct nlattr *opt) {
  ...
  if (tb[TCA_NETEM_CORR]) get_correlation(q, tb[TCA_NETEM_CORR]);

  if (tb[TCA_NETEM_REORDER]) get_reorder(q, tb[TCA_NETEM_REORDER]);

  if (tb[TCA_NETEM_CORRUPT]) get_corrupt(q, tb[TCA_NETEM_CORRUPT]);

  if (tb[TCA_NETEM_RATE]) get_rate(q, tb[TCA_NETEM_RATE]);
...
static int netem_init(struct Qdisc *sch, struct nlattr *opt) {
  ...
  if (ret) pr_info("netem: change failed\n");
  return ret;
}
...
static int netem_dump(struct Qdisc *sch, struct sk_buff *skb) {
  ...
  if (q->rate >= (1ULL << 32)) {
    if (nla_put_u64(skb, TCA_NETEM_RATE64, q->rate)) goto nla_put_failure;
    rate.rate = ~0U;
  } else {
    rate.rate = q->rate;
  }
...
static unsigned long netem_get(struct Qdisc *sch, u32 classid) { return 1; }
...
module_init(netem_module_init) module_exit(netem_module_exit)
    MODULE_LICENSE("GPL");
