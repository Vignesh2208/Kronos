...
SYSCALL_DEFINE1(stime, time_t __user *, tptr) {
  ...
  if (get_user(tv.tv_sec, tptr)) return -EFAULT;
...
int do_sys_settimeofday(const struct timespec *tv, const struct timezone *tz) {
  ...
  if (tv && !timespec_valid(tv)) return -EINVAL;
  ...
  if (tv) return do_settimeofday(tv);
  return 0;
}
...
SYSCALL_DEFINE2(settimeofday, struct timeval __user *, tv,
  ...
  if (tv) {
    if (copy_from_user(&user_tv, tv, sizeof(*tv))) return -EFAULT;

    if (!timeval_valid(&user_tv)) return -EINVAL;
...
SYSCALL_DEFINE1(adjtimex, struct timex __user *, txc_p) {
   ...
   * may change
   */
  if (copy_from_user(&txc, txc_p, sizeof(struct timex))) return -EFAULT;
  ret = do_adjtimex(&txc);
...
time64_t mktime64(const unsigned int year0, const unsigned int mon0,
  ...
  return ((((time64_t)(year / 4 - year / 100 + year / 400 + 367 * mon / 12 +
                       day) +
            year * 365 - 719499) *
               24 +
           hour /* now have hours */
...
struct timespec64 ns_to_timespec64(const s64 nsec) {
  ...
  if (!nsec) return (struct timespec64){0, 0};
...
unsigned long __usecs_to_jiffies(const unsigned int u) {
  if (u > jiffies_to_usecs(MAX_JIFFY_OFFSET)) return MAX_JIFFY_OFFSET;
 ...
 */
static unsigned long __timespec64_to_jiffies(u64 sec, long nsec) {
...
static unsigned long __timespec_to_jiffies(unsigned long sec, long nsec) {
...
unsigned long timeval_to_jiffies(const struct timeval *value) {
  return __timespec_to_jiffies(value->tv_sec, value->tv_usec * NSEC_PER_USEC);
}
...
#if (HZ % USER_HZ) == 0
  if (x >= ~0UL / (HZ / USER_HZ)) return ~0UL;
  return x * (HZ / USER_HZ);
#else
