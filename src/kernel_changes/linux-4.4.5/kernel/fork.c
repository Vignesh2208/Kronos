...
static struct task_struct *dup_task_struct(struct task_struct *orig) {
	...
	tsk->ptrace_msteps = 0;
	tsk->n_ints = 0;
	tsk->freeze_time = orig->freeze_time;
...
long _do_fork(unsigned long clone_flags,
	...
	if (!IS_ERR(p)) {
		...
		if (unlikely(trace)) {
			trace_printk("Sending NEW CLONE/FORK event to tracer. Current task : %d, Event = %d\n", current->pid, trace);
			ptrace_event_pid(trace, pid);
			trace_printk("Resuming after CLONE/FORK event send. Current task : %d, Event content = %lu\n", current->pid, current->ptrace_message);
