...
long vtIoctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
    ...
    case VT_GET_TIME_TRACER:
      // Any process can invoke this call.

      if (initialization_status != INITIALIZED) {
        PDEBUG_E(
            "VT_GET_TIME_TRACER: Operation cannot be performed when experiment is "
            "not initialized !");
        return -EINVAL;
      }
      ...
      api_info_tmp.return_value = HandleGettimeTracer(api_info_tmp.api_argument);
      if (copy_to_user(api_info, &api_info_tmp, sizeof(invoked_api))) {
        PDEBUG_I("VT_GETTIME_PID: Error copying to user buf\n");
        return -EINVAL;
      }
